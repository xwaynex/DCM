(function ($) {
  ...
  $(window).scroll(function () {
    ...
    if ($(window).scrollTop() > 400) {
      $("#back-to-top")
        .stop()
        .animate({ bottom: "16px" }, 300, "easeInOutCubic");
    } else {
      $("#back-to-top")
        .stop()
        .animate({ bottom: "-50px" }, 300, "easeInOutCubic");
    }
  });
  /* -------------------
    Preloader
  ...
  $("#contactform").submit(function () {
    ...
    $("#message").slideUp(250, function () {
      ...
      $("#submit")
        .after(
          '<img src="../img/assets/contact-form-loader.gif" class="loader" />'
        )
...
var moreBtnOne = document.getElementById("moreBtnOne");
...
Array.prototype.forEach.call(more, attachingEvent);

//Get profile
var profiles = document.querySelectorAll(".profile");

//Get the link that open the profile
var profileBtn = document.querySelectorAll("button.profileBtn");

// Get button to close profile
...
for (var i = 0; i < profileBtn.length; i++) {
  profileBtn[i].onclick = function (e) {
    e.preventDefault();
    displayProfile = document.querySelector(e.target.getAttribute("href"));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("teamProfileOverlay")
    .addEventListener("click", closeProfileModal);
  document
    .getElementById("teamClose")
    .addEventListener("click", closeProfileModal);
});
    ...
    .then((countries) => {
      countries
        ...
        .forEach((c) => {
          ...
          opt.setAttribute("data-flag", c.flags.svg);
          select.append(opt);
        });
...
for (var i = 0; i < profileClose.length; i++) {
  profileClose[i].onclick = function () {
    for (var index in profiles) {
      if (typeof profiles[index].style !== "undefined")
        profiles[index].style.display = "none";
    }
  };
}

//when user clicks outside profile, close it
...
//Click the close button to close profile overlay
Array.prototype.forEach.call(profileClose, (button) => {
  button.addEventListener("click", () => {
    closeOverlay(profileOverlay);
  });
});
function closeOverlay(profileOverlay) {
